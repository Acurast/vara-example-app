...
export type JobStatus = "open" | "matched" | "assigned" | "finalizedOrCancelled";
...
export class Program {
  ...
  constructor(public api: GearApi, public programId?: `0x${string}`) {
    const types: Record<string, any> = {
      ConfigureArgument: {"_enum":{"Owner":"[u8;32]","IBCContract":"[u8;32]","AcurastPalletAccount":"[u8;32]","Paused":"bool","PayloadVersion":"u16","MaxMessageBytes":"u16","ExchangeRatio":"ExchangeRatio"}},
      ExchangeRatio: {"numerator":"u16","denominator":"u16"},
      UserAction: {"_enum":{"RegisterJob":"RegisterJobUserInput","DeregisterJob":"u128","FinalizeJob":"Vec<u128>","SetJobEnvironment":"SetJobEnvironmentUserInput","Noop":"Null"}},
      RegisterJobUserInput: {"job_registration":"JobRegistrationV1","destination":"[u8;32]","expected_fulfillment_fee":"u128"},
      JobRegistrationV1: {"script":"Vec<u8>","allowed_sources":"Option<Vec<[u8; 32]>>","allow_only_verified_sources":"bool","schedule":"ScheduleV1","memory":"u32","network_requests":"u32","storage":"u32","required_modules":"Vec<u16>","extra":"JobRequirementsV1"},
      ScheduleV1: {"duration":"u64","start_time":"u64","end_time":"u64","interval":"u64","max_start_delay":"u64"},
      JobRequirementsV1: {"assignment_strategy":"AssignmentStrategyV1","slots":"u8","reward":"u128","min_reputation":"Option<u128>"},
      AssignmentStrategyV1: {"_enum":{"Single":"Option<Vec<PlannedExecutionV1>>","Competing":"Null"}},
      PlannedExecutionV1: {"source":"[u8; 32]","start_delay":"u64"},
      SetJobEnvironmentUserInput: {"job_id":"u128","public_key":"Vec<u8>","processors":"Vec<SetJobEnvironmentProcessor>"},
      SetJobEnvironmentProcessor: {"address":"[u8;32]","variables":"Vec<(Vec<u8>, Vec<u8>)>"},
      Config: {"owner":"[u8;32]","ibc":"[u8;32]","acurast_pallet_account":"[u8;32]","paused":"bool","payload_version":"u16","max_message_bytes":"u16","exchange_ratio":"ExchangeRatio"},
      JobInformation: {"_enum":{"V1":"JobInformationV1"}},
      JobInformationV1: {"schedule":"ScheduleV1","creator":"[u8;32]","destination":"[u8;32]","processors":"Vec<[u8;32]>","expected_fulfillment_fee":"u128","remaining_fee":"u128","maximum_reward":"u128","status":"JobStatus","slots":"u8"},
      JobStatus: {"_enum":["Open","Matched","Assigned","FinalizedOrCancelled"]},
    }
  ...
  newCtorFromCode(code: Uint8Array | Buffer, owner: ActorId | null, ibc: ActorId | null): TransactionBuilder<null> {
  ...
  newCtorFromCodeId(codeId: `0x${string}`, owner: ActorId | null, ibc: ActorId | null) {
    const builder = new TransactionBuilder<null>(
      this.api,
      this.registry,
      'create_program',
...
export class VaraProxy {
  ...
  public configure(actions: Array<ConfigureArgument>): TransactionBuilder<null> {
  ...
  public fulfill(job_id: number | string | bigint, payload: `0x${string}`): TransactionBuilder<null> {
    if (!this._program.programId) throw new Error('Program ID is not set');
    return new TransactionBuilder<null>(
      this._program.api,
      this._program.registry,
  ...
  public async config(originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<Config> {
    const payload = this._program.registry.createType('(String, String)', ['VaraProxy', 'Config']).toHex();
  ...
  public async job(job_id: number | string | bigint, originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<JobInformation> {
    const payload = this._program.registry.createType('(String, String, u128)', ['VaraProxy', 'Job', job_id]).toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
      payload,
    ...
    if (!reply.code.isSuccess) throw new Error(this._program.registry.createType('String', reply.payload).toString());
    const result = this._program.registry.createType('(String, String, JobInformation)', reply.payload);
    return result[2].toJSON() as unknown as JobInformation;
  }

  public async nextJobId(originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<bigint> {
    const payload = this._program.registry.createType('(String, String)', ['VaraProxy', 'NextJobId']).toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
      payload,
